...
# Binary sytem to define type of pokemon: 1 has specific type - 0 does not have certain type
...
# Split Dataset in Test und Testsets
def train_test_splitter(DataFrame, column):
    # Split dataset in training and testing dataset: Trainset all above G1, Testset is G1
    df_train = DataFrame.loc[df[column] != 1]
    df_test = DataFrame.loc[df[column] == 1]
    
    # Drop column Generation from dataset
...
print('Our test accuracy was '+str(accuracy_value)+'')
...
predictor(test_data, test_labels, 5)